...
- 无共同版本（或版本列表为空）时服务端回**错误响应**，由客户端负责
  ...
  `Done()` 等待者。
- 服务端处理 `HELLO` 完全无状态：协商结果由客户端记录
  （`client.NegotiatedVersion()`），服务端不需要每连接握手标记，也就
  没有状态清理问题。
...
- **断线重连**：握手响应中下发 token（`HelloResponse` 加字段即可，版本
  协商已就位），重连后凭 token 恢复订阅表。
