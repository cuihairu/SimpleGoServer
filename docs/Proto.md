...
- **会话 TTL**：断开的会话保留 10 分钟（`resumed` 惰性检查 + 后台
  ...
  连接无论挂多久都不会被回收，只有真正静默的会话才过期。清扫在
  作废会话的同时把其传输从各 topic 成员集里移除、释放其持有的补发
  缓存（与 resume 的"换传输"同构，只是方向为删除）——客户端订阅后
  消失也不会在订阅表里留下幽灵条目。
...
- **服务端缓存**：每次 Publish 把编码后的帧存入该 topic 的重放缓存
  （每 topic 最近 64 条，超出淘汰最旧；全局另有 64 MiB 字节预算，
  压力下按字节占用最大者整 topic 逐出——条数上限并不等于内存上限，
  topic 数无界且单帧可达 1 MiB）。关键不变量：**订阅者集合为空但
  仍有会话记着该 topic 时，发布继续入缓存**——否则最后一个订阅者
  被剔除后的发布全部静默丢失，补发就名存实亡。
...
- **语义边界（best-effort，不保证不亏）**：缓存按条数封顶，断线窗口
  内错过超过 64 条时最旧的补不回来；全局内存压力下整个 topic 的缓存
  可能被逐出，该 topic 的补发能力随之消失——客户端发现补发序号与自己
  游标之间有跳变，即知道有缺口，可自行决定拉取或重置；会话过期（TTL）
  或显式 UNSUBSCRIBE 后，topic 不再被任何会话记着，缓存随之清理。
